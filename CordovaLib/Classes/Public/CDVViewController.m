...
- (UIView*)newCordovaViewWithFrame:(CGRect)bounds
{
    ...
    if (NSClassFromString(webViewEngineClass)) {
        ...
        if ([webViewEngineClass isEqualToString:@"CDVWKWebViewEngine"]) {
            //wk添加location事件回调扩展。
            id <CDVWebViewEngineProtocol,CDVWebViewEngineExtensionProtocol> extensionWkEngine = (id <CDVWebViewEngineProtocol,CDVWebViewEngineExtensionProtocol>)self.webViewEngine;
            extensionWkEngine.extensionDelegate = self;
        }
    } else {
        self.webViewEngine = [[NSClassFromString(defaultWebViewEngineClass) alloc] initWithFrame:bounds];
    }
